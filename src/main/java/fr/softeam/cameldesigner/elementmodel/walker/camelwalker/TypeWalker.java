...
public class TypeWalker extends AbstractCamelWalker {
    ...
    protected void walkChildren(Object processedElement, CamelElement resultedElement) {
        if( processedElement instanceof camel.type.TypeModel) {
            camel.type.TypeModel typeModel = (camel.type.TypeModel) processedElement;
            
            for(camel.type.ValueType valueType : typeModel.getValueTypes()) {
                this.reverseProcess.setUmlElementParent(resultedElement.getElement());
                walk(valueType);
            }        
            
            for(camel.type.Value value : typeModel.getValues()) {
                this.reverseProcess.setUmlElementParent(resultedElement.getElement());
                walk(value);
            }        
        }
