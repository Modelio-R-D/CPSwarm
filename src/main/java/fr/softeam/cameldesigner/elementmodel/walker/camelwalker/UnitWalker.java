...
public class UnitWalker extends AbstractCamelWalker {
    ...
    protected void walkChildren(Object processedElement, CamelElement resultedElement) {
        if( processedElement instanceof camel.unit.UnitModel) {
            camel.unit.UnitModel unitModel = (camel.unit.UnitModel) processedElement;
            
            for( camel.unit.Unit unit : unitModel.getUnits()) {
                this.reverseProcess.setUmlElementParent(resultedElement.getElement());
                walk(unit);
            }        
            
            for( camel.unit.UnitDimension unitDimension : unitModel.getDimensions()) {
                this.reverseProcess.setUmlElementParent(resultedElement.getElement());
                walk(unitDimension);
            }        
        }
