...
public class PortInTool extends DefaultBoxTool {
    /**
     * Default constructor
     */
    public PortInTool() {
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        try (ITransaction transaction = session.createTransaction("org.modelio.module.CPSwarm.command.diagram.PortInTool")) {
        
            MObject element = target.getElement();
        
            Port port = CPSwarmModule.getInstance().getModuleContext().getModelingSession().getModel().createPort();
            if (target.getElement() instanceof Instance) {
                port.setCluster((Instance) element);
            } else if (target.getElement() instanceof Classifier) {
