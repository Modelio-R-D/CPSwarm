...
import org.modelio.module.cpswarm.type.abstraction.AbstractionExporter;
...
/**
 * This class provides the ADF export dialog
 * @author ebrosse
 */
public class ADFExportWindow extends AbstractSwtWizardWindow {
    private void createContents() {
        ...
        this.fileChooserComposite.getSearch().addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                ADFExportWindow.this.fileChooserComposite.searchFile();
            }
        });
    ...
    public void validationAction() {
        ...
        try(ITransaction t = CPSwarmModule.getInstance().getModuleContext().getModelingSession().createTransaction("ADF Export") ) {
        
            AbstractionExporter exporter = new AbstractionExporter();
            exporter.exporting((org.modelio.metamodel.uml.statik.Class) this.selectedElt, theFile);
    ...
    public void setPath() {
        try {
            ...
            this.fileChooserComposite.setText(ResourcesManager.getInstance().getGeneratedPath() + File.separator + this.selectedElt.getName() + ".json" );
     ...
     */
    public ADFExportWindow(final Shell parent) {
        super(parent);
    }
